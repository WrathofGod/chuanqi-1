...
var check = function(serverData,sClient,cb){
    ...
    guildWarRecordDao.selectCols(sClient,"id,lastRankData"," 1=1 order by recordTime desc",[],function(err,data){
        ...
        var mailChairList = [];
        var mailUserList = [];
        for(var key in data.lastRankData){
            var locGroupId = key;
            var locLastRankData = data.lastRankData[key];
            var chairArr = locLastRankData.chairArr || [];
            for(var i = 0;i<chairArr.length;i++){
                var locChair = chairArr[i];
                if(locChair.serverId != serverData.serverId) continue;
                var locChairItems = getAwardItems(locGroupId,locChair.rank,2);

                var locMail = mailBiz.createEntityByType(locChair.userId, c_prop.mailTypeKey.guildWarRank2, [c_prop.guildGroup[locGroupId], locChair.rank], locChairItems);
                mailChairList.push(locMail);
            }

            var userArr = locLastRankData.userArr || [];
            for(var i = 0;i<userArr.length;i++){
                var locUser = userArr[i];
                if(locUser.serverId != serverData.serverId) continue;
                var locUserItems = getAwardItems(locGroupId,locUser.rank,3);
                var locMail = mailBiz.createEntityByType(locUser.userId, c_prop.mailTypeKey.guildWarRank3, [c_prop.guildGroup[locGroupId], locUser.rank], locUserItems);
                mailUserList.push(locMail);
            }
        }

        async.parallel([
            function(cb1){
                _sendChair(sClient,mailChairList,cb1);
            },
            function(cb1){
                _sendUser(sClient,mailUserList,cb1);
            }
        ],function(err,data){
            if(err) return cb(err);
...
var _sendChair = function(sClient,mailList,cb){
    var mailCount = 0;
    async.mapLimit(mailList,1, function (mail, cb1) {
        var usr = mail.userId;
        mailDao.selectCols(sClient,"id"," userId = ? and type = ?",[usr,c_prop.mailTypeKey.guildWarRank2],function(err,mailData){
            if(err) return cb1(err);
            if(mailData){
                cb1();
            }else{
                mailCount ++;
                mailDao.insert(sClient,mail,cb1);
            }
        });
    }, function(err,data){
        if(err) return cb(err);
...
var _sendUser = function(sClient,mailList,cb){
    var mailCount = 0;
    async.mapLimit(mailList,1, function (mail, cb1) {
        var usr = mail.userId;
        mailDao.selectCols(sClient,"id"," userId = ? and type = ?",[usr,c_prop.mailTypeKey.guildWarRank3],function(err,mailData){
            if(err) return cb1(err);
            if(mailData){
                cb1();
            }else{
